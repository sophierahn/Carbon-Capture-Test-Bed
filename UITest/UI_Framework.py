...
def UI_main(UI_pipe):   #add in UI_pipe
    if __name__ == '__main__':
        while UI_pipe.poll():
            sensor_up = UI_pipe.recv()
        root = Tk()
...
#so putting it in a while loop creates 50000 windows, no while loop makes it close immediately

#the internet told me to put in a if __name__ == '__main__' to run this on windows and avoid recursive processes
#obvi thats not great for this where it won't always be on windows, so theres gotta be a better way to treat the issue
#yeah putting in name == main introduces other deeper issues

#https://stackoverflow.com/questions/44635546/tkinter-separating-processes
#might want to give this a go on a virtual machine
